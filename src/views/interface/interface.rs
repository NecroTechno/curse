...
use cursive::views::{
    Button, Canvas, Dialog, DummyView, EditView, LinearLayout, ListView, NamedView, OnEventView,
    PaddedView, Panel, ResizedView, ScrollView, SelectView, TextView,
};
use cursive::Cursive;
...
pub fn interface(siv: &mut Cursive, state_manager: &'static Mutex<StateManager>) {
    ...
    let menu = generate_menu_view(state_manager);
    let sidebar = generate_sidebar_view(logo, menu, state_manager);
    let job_ui = generate_job_ui_view();
    ...
    let layout = PaddedView::lrtb(
        ...
        1,
        1,
        LinearLayout::horizontal().child(job_ui).child(sidebar),
    );
...
fn generate_sidebar_view(
    logo: NamedView<TextView>,
    menu: NamedView<LinearLayout>,
    ...
    return LinearLayout::vertical()
        ...
        .child(ResizedView::with_full_height(
            // Notifications panel
            Panel::new(ListView::new().with_name(NOTIFICATION_VIEW_NAME)).title("Notifications"),
        ));
}

fn generate_menu_view(state_manager: &'static Mutex<StateManager>) -> NamedView<LinearLayout> {
